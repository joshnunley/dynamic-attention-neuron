...
# TODO:
# 1. This model is likely heavily overfitting, it doesn't need the correct input to predict
# just some dynamic input (though it may need certain characteristics like oscillation and
#  some sort of difference for each neuron)
...
class DynamicAttentionNetwork(nn.Module):
    def __init__(
        ...
        # self.state_update = StateUpdateFunction(2*neuron_state_size, hidden_layer_size, hidden_state_size)
        self.state_update = nn.Linear(
            2 * neuron_state_size, hidden_state_size, bias=True
        )
    ...
    def forward(
        ...
        neuron_predecessor_states = torch.cat(
            (neuron_states, accumulated_predecessor_states), 2
        )
        updated_hidden_states = hidden_states + self.step_size * self.state_update(
            neuron_predecessor_states
        )
...
class AccumulatorFunction(nn.Module):
    ...
    def forward(self, neuron_states):
        ...
        accumulated_predecessor_states = torch.matmul(
            self.adjacency_mask * relevancies, neuron_states
        )
